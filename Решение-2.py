...
sale17 = Sale (stock_id=17, price=530, date_sale = datetime(2022,11,6), count= 2)
...
# for s in que:
#     print(f' {s.Publisher.name} | {s.Book.title} | {s.Shop.name}| {s.Sale.price} | {s.Sale.date_sale}' )


def query_func (publisher_data):
    ''' Функция принимает на вход введённые пользователем : либо имя автора ,
    либо его id из таблицы Publisher.
    На выходе получаем таблицу со столбцами :
    название книги | название магазина, в котором была куплена эта книга |
    стоимость покупки | дата покупки'''
    print()
    if publisher_data.isdigit() :
        for s in que.filter(Publisher.id == publisher_data).all():
            print(f' {s.Book.title:<25} | {s.Shop.name:<13} | {s.Sale.price:^7} | {s.Sale.date_sale.strftime("%d-%m-%Y")}')
    else :
        for s in que.filter(Publisher.name == publisher_data).all():
            print(f'{s.Book.title: <25} | {s.Shop.name:<13} | {s.Sale.price:^7} | {s.Sale.date_sale.strftime("%d-%m-%Y")}')

publisher_data = input("Введите id автора либо его фамилию: ")

if __name__ == '__main__':
    query_func(publisher_data)
